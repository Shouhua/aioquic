...
function
function执行后，variable全局可见；可以使用local，对自己和子function可见

shell parameter
positional parameters 数组大于1位数必须使用{}, 比如${10}, ${11}
set -- 15 14 13 12 11 10 9 8 7 6 5 4 3
echo "$11 ${11}" # 151 5
special parameters
$* $@ 每个参数都是独立个体，并且个体使用word splitting和filename expansion,功能一样
IFS=A; set -- helloAworld again; echo $@ # hello world again
IFS=A; set -- helloAworld again; echo $* # hello world again
"$*" "$1c$2c..."使用IFS的第一个字符连接成a single word
"$@" "$1" "$2" ...
...
or cat -A <<<"$IFS" # https://bash.cyberciti.biz/guide/$IFS
"$@" = "$1" "$2" ... and $@没有引号时，受制于word splitting，即使用IFS分割参数
```
IFS="A"
for v in $@; do echo $v done # 如果参数是helloAworld again，结果是hello\nworld\nagain，word splitting以前会先使用空格分隔
```
$$ process id of the shell, in subshell, it expands to the process id of the invoking shell, not the subshell.
$BASHPID 代表当前shell的pid
$! 上一个bg的pid

shell expansions
brace expansion echo a{b,c,d}e  {x..y[..incr]} x and y are either int or letters, incr int
tidle expansion ~ = $HOME, ~+ = $PWD, ~- = $OLDPWD, ~fred/foo The subdirectory foo of the home directory of the user fred
shell parameter expansion 
$ char introduces parameter expansion, command substitution, or arithmetic expansion
parameter expansion $0 $* $@ ${parameter:-word}, 
indirect expansion ${!prefix@} 返回以prefix开头的变量，以IFS的第一个字符连接，这2个除外${!array[*]}, ${!array[@]} 获取数组keys
echo ${!LC_@} # LC_ADDRESS LC_IDENTITICATION ...
$- current set/bash options flags
$_ 最近一个前台命令的最后一个参数

substring expansion ${parameter:offset:length}, ${expression/regex/substr}
...
quote removal
顺序执行，bash options或者set可以控制是否执行上述expansions

redirection
&>word equals to >word 2>&1
here document
eval $'cat <<-"EOF" > test.txt\n\thome:$OLDPWD\nEOF'
如果EOF有引号，内容不会展开 test.txt内容为home: $HOME；<<-会清除leading spaces
here string
cat <<< ~ # /home/shouhua

execute command -> simple command expansion(https://www.gnu.org/software/bash/manual/bash.html#Simple-Command-Expansion)
执行规则
1. 先保存赋值语句和redirections
2. expansion，确定command and arguments
...
break [n]/exit [n]/return [n]
eval
exec
trap(https://www.junmajinlong.com/shell/script_course/shell_trap/index.html)
trap错误时，$BASH_LINENO和$BASH_COMMAND比较有用
Bash(Bourne Again Shell) Builtins
declare
echo
local
mapfile Read lines from the standard input into the indexed array variable array, or from file descriptor fd if the -u option is supplied. The variable MAPFILE is the default array
printf [-v var] format [arguments] assign result to variable var
read
while read name; do
	if [[ -z $name ]]; then
		echo "name is empty"
...
set builtin
1. change the values of shell options, set -e -u -o pipefail
2. set the positional parameters, set -- or set -
3. display the names and values of shell variables
set -x print executed command lines
set -H eanble '!' style history subsitution. This option is on by default for interactive shells
但是在bash script中默认没有-H，所有我们可以使用${!parameter}

declare除了生成变量，还可以设置属性，比如只读，只为int等(https://unix.stackexchange.com/questions/254367/in-bash-scripting-whats-the-different-between-declare-and-a-normal-variable)
declare -r name=value # readonly
declare -i name=value # integer only for name
declare -p IFS # 显示IFS的定义
delcare -x name=value # export name=value
declar -n name=nameref # name=${!nameref}

array
declare -a name indexed array
name=(value1 value2 ... )
declare -A name associated array
name=(key1 value1 key2 value2 ... )
or name=([key1]=value1 [key2]=value2 ... )
unset name[key1]
unset name
${!name[@]} keys of name
